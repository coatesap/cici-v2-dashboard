...
const ListKnowledgeItems = () => {
    ...
    const handleDelete = (item) => {
        ...
        fetch(`/api/delete-knowledge-item?id=${item.id}`)
            .then(res => {
                if (res.status === 204) {
    ...
    return (
        <div className="min-h-screen" style={{backgroundColor: '#E8E6E5'}}>
            <Head>
                <title>Knowledge items - CiCi Dashboard</title>
            </Head>
            <Header/>
            ...
            <div className="mx-auto max-w-6xl px-4 mt-4 pb-10">
                ...
                <ScreenHeader
                    ...
                    rightSlot={<AnchorButton
                        type="submit"
                        intent="primary"
                        text="Add Knowledge Item"
                        icon="cube-add"
                        onClick={() => router.push(`/knowledge-items/new`)}
                    />}
                />
                    ...
                    <div>
                        ...
                        <table className="mt-4">
                            ...
                            <tr className="border-b border-gray-300 bg-gray-300">
                                ...
                                <th className="px-4 py-1 text-left">Region</th>
                                <th className="px-4 py-1 text-left">Audience</th>
                            ...
                            {items.filter(filterItem).map(item => (
                                <tr key={item.id} className="border-b border-gray-300 hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-1 text-xs">{item.region}</td>
                                    <td className="px-4 py-1 text-xs">{item.audience}</td>
...
export default withAdminUser(ListKnowledgeItems)
